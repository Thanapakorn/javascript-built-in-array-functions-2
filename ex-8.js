...
function ProvineTotalPrice(bills) {
  return bills.reduce((acc, cur) => {
    const totalPaidByLocation = cur.location;

    if (!acc[totalPaidByLocation]) {
      acc[totalPaidByLocation] = cur.total;
    } else {
      acc[totalPaidByLocation] += cur.total;
    }
...
console.log(ProvineTotalPrice(bills));
